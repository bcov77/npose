...
# _atom = "ATOM".encode()
_null_line = (
    "ATOMCB00000000000000000000000000000000000000000000000000000000"
)  # .encode()
...
@njit(fastmath=True, cache=True)
<<<<<<< HEAD
def read_npose_from_data(
    data, null_line_atom_names, NCACCBR, scratch_residues, scratch_chains
):
=======
def read_npose_from_data( data, null_line_atom_names, NCACCBR, scratch_residues, scratch_chains, scratch_aa):

>>>>>>> 954b5407ab03b15a119ec4087a3a1b0d683eac5c
...
>>>>>>> 954b5407ab03b15a119ec4087a3a1b0d683eac5c
                    ...
                    # Fixing CB
                    xform = get_stub_from_n_ca_c(
                        res[N, :3], res[CA, :3], res[C, :3]
                    )
...
<<<<<<< HEAD
        if res[atomi, 3] != 0:
            err_string = (
                "duplicate atom: "
            )  # + atom_name + " in residue: " + res_ident + " at line: " #+ str(iline)
            assert False
=======
        if ( res[atomi,3] != 0 ):
...
<<<<<<< HEAD
    npose, scratch, scratch_chains = read_npose_from_data(
        data,
        _null_line_atom_names,
        NCACCBR,
        g_scratch_residues,
        g_scratch_chains,
    )
...
def get_just_my_atoms(pdpose):
    return pdpose[
        pdpose.an.str.contains("^(%s)$" % ("|".join(ATOM_NAMES)))
    ].copy()


def add_my_resnum(just_my_atoms):
    just_my_atoms["my_resnum"] = (
        just_my_atoms["ri"] != just_my_atoms["ri"].shift(1).fillna(-1)
    ).cumsum()
...
def atom_thing(just_my_atoms):
    ...
    just_my_atoms["ind"] = just_my_atoms["my_resnum"] * R + just_my_atoms[
        "an"
    ].map(the_map)
...
def appenddd(just_my_atoms, cb_rows):
    if len(cb_rows) > 0:
        just_my_atoms = actual_append(
            just_my_atoms, cb_rows
        )  # just_my_atoms.append(cb_rows)
        just_my_atoms = actual_sort(
            just_my_atoms
        )  # just_my_atoms.sort_values('ind')
    return just_my_atoms
...
def build_CB(tpose):
    CB_pos = np.array(
        [0.52892494, -0.77445692, -1.19923854, 1.0], dtype=np.float
    )
    return tpose @ CB_pos
...
def build_H(npose):
    ...
    #                                    -cos(120)                      sin(120)
    to_first_c = (
        first_CA_to_N * 0.4999999999999998 + C_sin_vect * 0.8660254037844387
    )
...
def dump_npdb(
    npose,
    fname,
    atoms_present=list(range(R)),
    pdb_order=_pdb_order,
    out_file=None,
):
...
def add_to_score_file(
    tag, fname, write_header=False, score_dict=None, string_dict=None
):
    with open(fname, "a") as f:
        add_to_score_file_open(tag, f, write_header, score_dict, string_dict)


def add_to_score_file_open(
    tag, f, write_header=False, score_dict=None, string_dict=None
):
...
def add_to_silent(
    npose, tag, fname, write_header=False, score_dict=None, string_dict=None
):
    with open(fname, "a") as f:
        add_to_silent_file_open(
            npose, tag, f, write_header, score_dict, string_dict
        )

<<<<<<< HEAD

def add_to_silent_file_open(
    npose, tag, f, write_header=False, score_dict=None
):
    final_dict = get_final_dict(score_dict, string_dict)
    if write_header:
=======
...
def calc_rmsd(npose1, npose2):
    ...
    return math.sqrt(
        np.sum(
            np.square(np.linalg.norm(npose1[:, :3] - npose2[:, :3], axis=-1))
        )
        / (len(npose1))
    )
...
def clashgrid_from_tpose(tpose, atom_size, resl, padding=0):
    return clashgrid_from_points(
        points_from_tpose(tpose), atom_size, resl, padding
    )
...
def nearest_object_grid(
    objects, atom_size=3.5, resl=0.25, padding=0, store=None
):
    ...
    nearest_object = voxel_array.VoxelArray(
        low, high, np.array([resl] * 3), np.int32
    )
    nearest_dist = voxel_array.VoxelArray(
        low, high, np.array([resl] * 3), np.float32
    )
...
def npose_to_motif_hash_frames(npose):
    ...
    cen = (xforms_from_four_points(CAs, Ns, CAs, Cs) @ CEN[..., None]).reshape(
        -1, 4
    )
...
def get_CB_from_xform(xform):
    CB_pos = np.array(
        [0.52892494, -0.77445692, -1.19923854, 1.0], dtype=np.float32
    )
    return xform @ CB_pos
...
def rotate_npose_phi(npose, tpose, resno, delta_phi):
    phi_xform = get_phi_rotation_xform(
        tpose[resno], delta_phi, npose[resno * R + CA]
    )
...
def rotate_tpose_phi(tpose, resno, delta_phi):
    phi_xform = get_phi_rotation_xform(
        tpose[resno], delta_phi, tpose[resno, :, 3]
    )
    return apply_dihedral_to_xforms(
        tpose, phi_xform, resno
    )  # This affects resno xform


def rotate_npose_psi(npose, tpose, resno, delta_psi):
    psi_xform = get_psi_rotation_xform(
        tpose[resno], delta_psi, npose[resno * R + CA]
    )
...
def rotate_tpose_psi(tpose, resno, delta_psi):
    psi_xform = get_psi_rotation_xform(
        tpose[resno], delta_psi, tpose[resno, :, 3]
    )
...
def set_ca_phi(points, tpose, phis, resno, phi):
    return set_phi(
        points, tpose, phis, resno, phi, 1, 0, 0, 0
    )  # This affects resno
...
def set_phi(points, tpose, phis, resno, phi, local_R, R_off, R_CA, t_off):
    ...
    phi_xform = get_phi_rotation_xform(
        tpose[resno], delta_phi, points[resno * local_R + R_CA]
    )
    new_points = apply_dihedral_to_points(
        points, phi_xform, resno * local_R + R_off
    )
...
def set_psi(points, tpose, psis, resno, psi, local_R, R_off, R_CA, t_off):
    ...
    psi_xform = get_psi_rotation_xform(
        tpose[resno], delta_psi, points[resno * local_R + R_CA]
    )
    new_points = apply_dihedral_to_points(
        points, psi_xform, resno * local_R + R_off
    )
...
def prepare_context_by_dist_and_limits(
    context, pt, max_dist, unit_vector=None
):
    pt = pt[:3]
...
@njit(fastmath=True)
def clash_check_points_context(
    pts,
    point_dists,
    context_by_dist,
    context_dist_limits,
    ...
    for ipt in range(len(pts)):
        pt = pts[ipt]
        lo_limit = context_dist_limits[
            max(0, int(point_dists[ipt] - clash_dist - 1 - tol))
        ]
        limit = context_dist_limits[int(point_dists[ipt] + clash_dist + tol)]
        context = context_by_dist[lo_limit:limit]
        clashes += np.sum(
            np.sum(np.square(pt - context), axis=1) < clash_dist2
        )
...
def mm2(inv_xform, xforms, traces, trans_err2):
    ...
    trans_err2[:] = np.square(
        np.sum(a[0, :3] * b[:, :3, 3], axis=-1) + a[0, 3]
    )
...
def cluster_xforms(
    close_thresh, lever, xforms, inverse_xforms=None, info_every=None
):
    ...
    while np.sqrt(min_distances.max()) > close_thresh:
        ...
        if not info_every is None:
            if len(center_indices) % info_every == 0:
                print(
                    "Cluster round %i: max_dist: %6.3f  "
                    % (len(center_indices), np.sqrt(min_distances[cur_index]))
                )
...
def KMGT(x, w=3, d=1):
    if x < 1e3:
        return _F(w, d, x / 1e0)
    if x < 1e6:
        return _F(w, d, x / 1e3) + "K"
    if x < 1e9:
        return _F(w, d, x / 1e6) + "M"
    if x < 1e12:
        return _F(w, d, x / 1e9) + "G"
    if x < 1e15:
        return _F(w, d, x / 1e12) + "T"
    if x < 1e18:
        return _F(w, d, x / 1e15) + "P"
    if x < 1e21:
        return _F(w, d, x / 1e18) + "E"
    if x < 1e24:
        return _F(w, d, x / 1e21) + "Z"
    else:
        return _F(w, d, x / 1e24) + "Y"
...
def npose_dssp_helix(npose, hbond_cutoff=-0.1):
    ...
    hbond_i_ip4 = (
        fast_hbond(Hs[4:], don_rays[4:], Os[:-4], acc_rays[:-4], 0.5)
        < hbond_cutoff
    )
...
def npose_dssp_beta_turn(npose, hbond_cutoff=-0.0):
    """
    """
    ...
    hbond_i_ip3 = (
        fast_hbond(Hs[3:], don_rays[3:], Os[:-3], acc_rays[:-3], 0.75)
        < hbond_cutoff
    )
    res_1_3 = np.zeros(nsize(npose), np.bool)
    # res_1_3[:-3] = hbond_i_ip3
    res_1_3[1:-2] |= hbond_i_ip3
    res_1_3[2:-1] |= hbond_i_ip3
    return res_1_3


def npose_dssp(
    npose,
    hbond_cutoff_sheet=-0.1,
    hbond_cutoff_helix=-0.1,
    hbond_cutoff_turns=-0.0,
):
    ...
    acc_rays = Os - Cs
    acc_rays /= np.linalg.norm(acc_rays, axis=-1)[:, None]
    tiler = (acc_rays.shape[0], *[1] * (len(acc_rays.shape) - 1))
    repeat_factor = don_rays.shape[0]
    tiled_don_rays = np.tile(don_rays, tiler)
    tiled_Hs = np.tile(Hs, tiler)
    repeat_acc_rays = np.repeat(acc_rays, repeat_factor, axis=0)
    repeat_Os = np.repeat(Os, repeat_factor, axis=0)

    all_x_all_hbonds = (
        fast_hbond(tiled_Hs, tiled_don_rays, repeat_Os, repeat_acc_rays, 0.5)
        < hbond_cutoff_sheet
    )
    helix = npose_dssp_helix(npose, hbond_cutoff=hbond_cutoff_helix)
    not_helix = helix == False
    beta_turn = npose_dssp_beta_turn(npose, hbond_cutoff=hbond_cutoff_turns)
    not_beta_turn = beta_turn == False
    # print(list(zip(not_beta_turn, np.arange(nsize(npose)))))
    hbonds = (
        (
            np.count_nonzero(
                all_x_all_hbonds.reshape((repeat_factor, repeat_factor)),
                axis=-1,
            ).astype(np.bool)
            | np.count_nonzero(
                all_x_all_hbonds.reshape((repeat_factor, repeat_factor)),
    ...
    bulges = np.zeros_like(hbonds, np.bool)
    hbond_AND__p_OR_m = np.zeros_like(hbonds, np.bool)
    plus_one = np.zeros_like(hbonds, np.bool)
    plus_one[:-1] = hbonds[1:]  # & hbonds[:-1]
    not_hbond = hbonds == False
    minus_one = np.zeros_like(hbonds, np.bool)
    minus_one[1:] = hbonds[:-1]
    # print(list(zip(hbond_AND__p_OR_m, np.arange(hbond_AND__p_OR_m.shape[0]))))
    for n in range(1, 4):

        plus_n = np.zeros_like(hbonds, np.bool)
        plus_n[:-n] = hbonds[n:]  # & hbonds[:-n]
        minus_n = np.zeros_like(hbonds, np.bool)
        minus_n[n:] = hbonds[:-n]  # hbonds[:-1] & hbonds[1:]

        p1_AND_mn = plus_one & minus_n & not_helix
        pn_AND_m1 = plus_n & minus_one & not_helix
        bulges = (p1_AND_mn) | (pn_AND_m1) | bulges
        # print(list(zip(bulges, np.arange(bulges.shape[0]))))
        # print(list(bulges))
    bulges &= not_beta_turn
    pre_sheet = bulges | hbond_AND__p_OR_m
    hbonds_OR_bulges = hbonds | bulges
    bulges_plus_one = np.zeros_like(hbonds_OR_bulges, np.bool)
    bulges_plus_one[:-1] = hbonds_OR_bulges[1:]
    bulges_minus_one = np.zeros_like(hbonds_OR_bulges, np.bool)
    bulges_minus_one[1:] = hbonds_OR_bulges[:-1]
    p1_OR_m1 = bulges_plus_one | bulges_minus_one
    hbond_AND__p_OR_m = p1_OR_m1 & hbonds_OR_bulges
    sheet = hbond_AND__p_OR_m | pre_sheet
    # _fill_in_gaps(sheet)

    return helix, sheet, beta_turn


def npose_dssp_sheet(npose, hbond_cutoff=-0.1):
    """
    Takes npose, uses hbonds/(!helix) to find sheets

    Takes all X all to check for hbonds
    """

    return npose_dssp(npose, hbond_cutoff_sheet=hbond_cutoff)[0]
...
<<<<<<< HEAD
            ncaco = silent_tools.sketch_get_atoms(
                structure, [0, 1, 2, 3], [0]
            ).reshape(-1, 4, 3)
=======
            if ( aa ):
...
>>>>>>> 954b5407ab03b15a119ec4087a3a1b0d683eac5c
            ...
            for atom in ATOM_NAMES:
                if atom == "CB":
                    tpose = get_stubs_from_n_ca_c(
                        ncaco[:, 0], ncaco[:, 1], ncaco[:, 2]
                    )
                    cbs = build_CB(tpose)
